...
namespace PLEApp
{
    public class Canvas
    {
        ...
        public Graphics g;      //graphics to draw on

        public int x, y;        //x and y position

        public bool fill;       //fill status for shapes

        public Color color;     //pen and fill color

        public Canvas()
        {
            Bitmap b = new Bitmap(700,400);         //deafult bitmap
        ...
        {
            if (param.Length != 4)              //if invalid param length
                return false;
            x = param[2];
            y = param[3];
            return true;
        ...
        {
            param[0] = param[0].Trim().ToLower();       //trim and lowercase parameter
            if (param.Length != 1)                      //if invalid param length
                return false;
