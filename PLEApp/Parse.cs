...
namespace PLEApp
{
    public class Parse
    {
        DataTable dt;   //used to compute expressions
        Dictionary<String, String> variables;
        int[,] loopRecorder;        //number of loop and its starting and ending line (basically stores the desired line number)
        String[] loopCondiition;    //stores the condition of the loop according to index order of recorder
        ...
        int totalLines;
        bool ifRunning;          //stops the parsing of line if if block is iterating and condition was false
        bool loopRunning;           //stops the parsing of line if while block is running
        bool methodRunning;      //stops the parsing of line if method block is running

        //default constructor
        public Parse()
        {
            ifRunning = false;
            loopCondiition = new String[10];        //stores the condition of the loop according to index order of recorder
            ifRunning = false;
            methodRunning = false;
            dt = new DataTable();
            ...
            this.loopRecorder = new int[10, 2];         //number of loop and its starting and ending line
            for (int i = 0; i < 10; i++)
            {
        ...
        public void runCommand(String line, int lineNo)
        {
            ...
            switch (command)
            {
                ...
                case "method":
                    ...
                    methods.Add(uniqueMethodName, this.methodParameters); //Add method to dictionary   
                ...
                default:    //check for variable or method name
                    ...
                    if (line.Split('=').Length == 2)
                    {
                        ...
                        if (this.variables.ContainsKey(varName))  //Reassign a existing variable
                            this.variables[varName] = rhs;
                        else                                //create and store a new variable
                    ...
                    else if (line.Split('(').Length == 2)           //check for method call
                    {
                        ...
                        {
                            if (key1[1].Equals(calledMethodName))     //if method name matches with any stored method
                            {
                                match = true;
        ...
        {
            line = line.Trim().ToLower();               //remove trail spaces and convert to lower
            String[] words = line.Split(' ');           //splits the line by spaces between
        ...
        public void parseLine(StringBuilder errorList, Canvas displayCanvas, String line,  int numLine)
        {
            ...
            else if(words.Length == 2)
            {
                ...
                int[] intParam = new int[param.Length + 2];           //2 additional length for defining cursor position
                ...
                for (int i = 2; i < intParam.Length; i++)
                {
                    ...
                    try
                    {
                        intParam[i] = int.Parse(param[i-2]);        //convert string param to int
                    }
                    catch (Exception)
                    {
